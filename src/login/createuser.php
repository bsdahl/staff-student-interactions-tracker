...
$pw1 = $_POST['password1'];
$pw2 = $_POST['password2'];
$verifyemail = (isset($admin_email)) ? $admin_email : $newemail;
$successmsg = (isset($admin_email)) ? $requiresadminapproval : $signupthanks;
...
if ($pw1 != $pw2) {
    ...
    if (isset($_POST['newuser']) && !empty(str_replace(' ', '', $_POST['newuser'])) && isset($_POST['password1']) && !empty(str_replace(' ', '', $_POST['password1']))) {
        ...
        if ($response == 'true') {
            ...
            echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'. $successmsg .'</div><div id="returnVal" style="display:none;">true</div>';

            //Send verification email
            $m = new MailSender;
            $m->sendMail($verifyemail, $newuser, $newid, 'Verify');
